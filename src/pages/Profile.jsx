...
function Profile() {
  ...
  const loadProfile = async (uid, email) => {
    try {
      ...
      // 2. Load the user's ads
      const adsRef = collection(db, "ads");
      // NOTE: Change "userId" to "userEmail" below if your app uses email to link ads
      const q = query(adsRef, where("userId", "==", uid));
      const snapshot = await getDocs(q);

      const userAds = snapshot.docs.map((item) => ({
        id: item.id,
        ...item.data(),
      }));
