...
function Search() {
  ...
  useEffect(() => { loadAds(); }, [searchText, selectedCategory, selectedCondition, selectedSubcategory, selectedType]);

  const loadAds = async () => {
    try {
      setLoading(true);
      const snapshot = await getDocs(collection(db, "ads"));
      const allAds = snapshot.docs.map((item) => ({ id: item.id, ...item.data() }));

      const usersSnap = await getDocs(collection(db, "users"));
      const allUsers = usersSnap.docs.map((u) => ({ uid: u.id, ...u.data() }));
      setUsers(allUsers);
      ...
      const filteredAds = allAds.filter((ad) => {
        ...
        const matchesCondition = !selectedCondition || ad.condition?.toLowerCase() === selectedCondition.toLowerCase();
        const matchesSubcategory = !selectedSubcategory || ad.subcategory?.toLowerCase() === selectedSubcategory.toLowerCase();
        const matchesType = !selectedType || ad.type?.toLowerCase() === selectedType.toLowerCase();

        return matchesSearch && matchesCategory && matchesCondition && matchesSubcategory && matchesType;
      });
  ...
  const findSeller = (ad) => {
    if (!ad) return null;
    return users.find((u) =>
      u.uid === ad.userId || u.uid === ad.uid || u.uid === ad.ownerId ||
    ...
    <div className="cars-page">
      ...
      <div className="search-filter-bar">
        ...
        {(minPrice || maxPrice || sortBy !== "newest") && (
          <button className="search-clear-filters" onClick={() => { setMinPrice(""); setMaxPrice(""); setSortBy("newest"); }}>
            ✕ Clear filters
          </button>
        )}
      </div>
            ...
            return (
              <div className="car-card" key={ad.id}>
                <Link to={`/ad/${ad.id}`} className="car-image-link">
                  ...
                  {adImage ? (
                    <img
                      ...
                      className={`listing-photo ${isSold ? "sold-card-image" : ""}`}
                      loading="lazy"
                    />
                  ) : (
                ...
                <div className="car-info">
                  ...
                  <h2 className="car-price">ETB {Number(String(ad.price || 0).replace(/,/g, "")).toLocaleString("en-US")}</h2>

                  <Link to={seller ? `/user/${seller.uid}` : "#"} className="search-seller-row" onClick={(e) => { if (!seller) e.preventDefault(); }}>
                    <div className="search-seller-avatar">
                      {seller?.imageUrl ? (
                        <img src={seller.imageUrl} alt={sellerName} loading="lazy" />
                      ) : (
                  ...
                  <Link to={`/ad/${ad.id}`} className="car-details-link">👁 View Details</Link>
                </div>
              </div>
            );
          })}
